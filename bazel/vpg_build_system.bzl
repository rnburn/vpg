...
# with modifications.
def vpg_private_include_copts(includes, is_system=False):
    copts = []
    prefix = ''
...
def vpg_include_copts():
  return vpg_private_include_copts([
      "include",
      "src",
      "test",
  ]) + vpg_private_include_copts([
    "3rd_party/randutils/include", 
    "3rd_party/base64/include", 
  ], is_system=True)

def vpg_copts(is_3rd_party=False):
  if is_3rd_party:
    return [
      "-std=c++11",
    ]
...
def vpg_cc_library(name,
  ...
  native.cc_library(
      ...
      hdrs = hdrs,
      copts = vpg_include_copts() + vpg_copts(is_3rd_party) + copts,
      linkopts = linkopts,
      includes = includes,
      deps = external_deps + deps,
      data = data,
      linkstatic = 1,
      include_prefix = vpg_include_prefix(native.package_name()),
...
def vpg_cc_binary(
    ...
    native.cc_binary(
        ...
        srcs = srcs,
        data = data,
        copts = vpg_include_copts() + vpg_copts() + copts,
        linkopts = linkopts,
